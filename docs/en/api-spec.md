...
> **Note:** Use the value of `access_token` as the Bearer token in subsequent requests.
...
| statusFilter | string | Filter by exact status: `TODO`, `IN_PROGRESS`, or `DONE` |
...
> **Note:** `meta.page` is a number. `statusFilter` performs an exact, case-insensitive match.
...
| status | string | no | Must be `TODO`, `IN_PROGRESS`, or `DONE` |
...
Valid status transitions: any status can be changed to any other status value.
